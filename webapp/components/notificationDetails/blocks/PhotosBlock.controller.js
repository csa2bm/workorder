sap.ui.define([
	...
	return Controller.extend("com.twobm.mobileworkorder.components.notificationDetails.blocks.PhotosBlock", {
		...
		onCapturePhoto: function() {
			...
			if (oNav) {
				oNav.getPicture(function(file) {
						...
						var dataCreate = {
							NotifNo: this.getView().getBindingContext().getObject().NotifNo,
							Data: file,
							CreateDate: new Date(),
							CreatedBy: this.getView().getModel("appInfoModel").getData().UserFullName
						};

						var createPath = this.getView().getBindingContext().getPath() + "/NotifAttachmentsSet";
				...
				sap.m.MessageToast.show(this.getI18nText("NotificationDetails-CameraNotAvailable"));
			}
		},
				...
				onClose: function(oAction, object) {
					if (oAction === sap.m.MessageBox.Action.YES) {
						...
						if (this._oPopover.getModel("ImageModel").getData().LocalObjectUri) {
							...
							// 
							deletePath = "/NotifAttachmentsSet(AttachmentID='" + attachmentId + "',NotifNo='" + notifNo + "')";
						}
		...
		createPhotoViewerPopover: function() {
			if (!this._oPopover) {
				this._oPopover = sap.ui.xmlfragment("com.twobm.mobileworkorder.components.notificationDetails.fragments.PhotoViewerPopover",
					this);
